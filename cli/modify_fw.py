...
def modify_firmware(original_firmware, modified_firmware, key_remap, keymap_csv, unmapped_keys_csv):
    ...
    :param keymap_csv: Path to the CSV file containing key offsets and XORed IDs
    :param unmapped_keys_csv: Path to the CSV file containing XORed IDs
    """
    ...
    for source_key, target_key in key_remap.items():
        byte_offset, expected_xored_id = keymap[source_key]
        _, new_xored_id = keymap[target_key]

        if not byte_offset:
            sys.exit(f"Error: Key '{source_key}' is not mapped in the original keyboard.")      
        if data[byte_offset] != expected_xored_id:
            sys.exit(f"Error: Unexpected value at {hex(byte_offset)}. Expected {hex(expected_xored_id)}, found {hex(data[byte_offset])}.")
