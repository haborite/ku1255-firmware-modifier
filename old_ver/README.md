# About
This repository provides a keymap for the Lenovo KU-1255 (ThinkPad Compact USB Keyboard with TrackPoint) firmware installer. The firmware installer stores key-specific values, generated by XORing the HID Usage ID with 0x5A, at specific offsets within the binary. By modifying these XORed IDs, you can remap any key on the KU-1255. A GUI tool to easily create a key-remapped firmware installer is also provided.
...
<img width="640" alt="gui-overview" src="https://github.com/haborite/ku1255-firmware-modifier/blob/main/python_ver/img/gui-overview.png">

5. Click "1. Select Original Firmware" and select the downloaded firmware installer `tp_compact_usb_kb_with_trackpoint_fw.exe`.
...
5. Click "3. Modify and Save New Firmware", and specify the filename and path of the new firmware installer.
...
# Address & XORed value of ID
Only keys in `Keyboard/Keypad Page (0x07)` are listed. Only keys with `Address` values are originally assigned in KU-1255. 
| ID | Address | XORed_ID | Usage Name | 0B47190 (US) | 0B47208 (JIS) | ISO |
| --- | --- | --- | --- | --- | --- | --- |
| 00 |  | 5A | No Event |  |  |  |
| 01 |  | 5B | ErrorRollOver |  |  |  |
| 02 |  | 58 | POSTFail |  |  |  |
...
# Structure of firmware installer
Note that `Address` shown in the above table is the address in the "firmware installer" not in the original "firmware".
The firmware is XOR-encrypted with the key `0x5A` and stored in the position of `0x73570` of the firmware installer.
