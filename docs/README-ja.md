...
5. **TrackPoint Speed**  
   トラックポイント速度をOS側とは独立に上昇させることができます（デフォルト: 1）。
   - トラックポイント速度を調節する際は、まずは(1)Lenovo公式ドライバ設定のポインタ速度、(2)OS側のマウス速度設定を優先することをお勧めします。それらの両方の速度をMAXにしても満足な速度が得られない場合に、(3)このファームウェアカスタマイズを検討してください。
